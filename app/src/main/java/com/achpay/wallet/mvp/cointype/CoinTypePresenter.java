...
public class CoinTypePresenter extends BasePresenter<CoinTypeView> {
    private DownloadChangeObserver observer;
    private Activity mContext;
    private String amount;
    private String currency;

    public CoinTypePresenter(CoinTypeView view) {
        super(view);
    }
    ...
    public void checkLoginInfo(Activity mContext) {
        ...
        if (uri != null) {//由第三方APP跳转过来携带的信息
            ...
            if (!CommonUtil.isSupportedCurrency(mContext, currency)) {
                Toast.makeText(mContext, mContext.getString(R.string.currency_not_supported), Toast.LENGTH_SHORT).show();
                return;
            }

            if (MvpRef != null) {
                MvpRef.get().showLoading();
                MvpRef.get().setAmount(amount);
                MvpRef.get().setCurrency(currency);
            }
            ...
            amount = intent.getStringExtra(TransParams.AMOUNT);
            currency = CommonUtil.getCurrencyId();
            if (MvpRef != null) {
                MvpRef.get().setAmount(amount);
                MvpRef.get().setCurrency(currency);
